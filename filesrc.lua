...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>NoHope Esport chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "NoHope Esport", HidePremium = false, IntroText = "NoHope Esport⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("NoHope Esport SCRIPT")
W:AddLabel("DiscordNoHope Esport")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "NoHope Esport(Noitification)",
